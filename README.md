...
**What this is.** For every US county (3,143), congressional district (435 + DC) and state: how many recruiting cancer treatment trials are within reach of residents aged 55+, how far they must drive to a broad menu of trials or to an NCI-designated cancer center, which trials those are, how many people are diagnosed with cancer there, and what household barriers (vehicle, broadband, insurance, poverty) sit alongside the distance. Version 3 brings the national map up to the rigour of the Texas road-network version: shortest highway routes instead of straight-line ×1.2, census-tract population weights instead of land-area weights, and adds trial lists, filters, a cancer-incidence layer and household context. Registry pulled 10 September 2026 (registry data timestamp 2026-09-10 09:00); the 9 September pull is kept as a snapshot and the difference is shown in the app.

Web app: `docs/` (static, deployable to GitHub Pages as-is; `index.html` map with Trials / Sites / Compare counties / What changed tabs, `brief.html` printable briefs — viewable by county, congressional district or state, with phase / sponsor / cancer-type filters that recompute county counts and the access figures and maps for districts, states and the nation at census-tract level —, `trials.html` per-county trial lists, `sites.html` the Tier 4 research prototype, plus `data.js`, `geo.js`, `tier4.js`, and `tracts60.bin` — a 6 MB census-tract file the page loads once, only when a filter is applied to a national, state or district view, so that filtered access figures are recomputed at tract level in the browser; d3 loads from cdnjs with a local fallback).

## Headline results (residents 55+, trials open to a 55-year-old, road miles from each census tract's population center)
- **12.8% of Americans 55+ have fewer than 20 recruiting cancer treatment trials within 60 road-miles of home; 4.6% have none.** (3,915 eligible trials on 2026-09-10.) (20.9% live in a county with no trial — a county-boundary statistic that overstates isolation and is no longer the lead number.)
...
| `docs/data.js`, `docs/geo.js`, `docs/tracts60.bin` | Data payload (4.9 MB), simplified geometry (2.3 MB), tract-level pools for live filtering (6.3 MB, loaded on demand) |
...
python diff_snapshots.py                        # 15. what changed vs the previous snapshot (data/snapshots/<date>/) -> docs/changes.js
python validate_v3.py                           # 16. re-runs every check (incl. 25 live registry re-fetches) -> VALIDATION_v3.md
# Refresh later: sh run_refresh.sh (steps 1,3,5-9,14 then 12,16); .github/workflows/refresh.yml runs it weekly on GitHub Actions (written, not yet executed — run it once manually first)
```
...
- **Trials, not eligibility.** Counts pool all cancer types unless filtered; the cancer-type filter is regex-based and 28.7% of trials sit in the multi-cancer bucket by design. Filters apply to county counts and lists and to the *access* figure (share with fewer than 20 / none / typical count within 60 road-miles) for the nation, states and districts, recomputed live from census tracts; distance-to-menu and distance-to-NCI-center figures depend on the full trial set and are shown tagged as unfiltered.
...
- **Snapshot.** One day's registry and one day's member list; re-pull before distributing. The NCI-designated center list (68 treating centers) is inherited from the pipeline QC and should be refreshed from cancer.gov — designations change (e.g. the Medical College of Wisconsin Cancer Center was seeking designation as of mid-2025).
