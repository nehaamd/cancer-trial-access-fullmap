...
def main():
    ...
    fac = sites.groupby(["county_fips", "fac"]).agg(name=("facility", "first"), city=("city", "first"), zip=("zip", "first"), trials=("nct_id", lambda s: sorted({j_of[n] for n in s}))).reset_index()
    fac["name"] = fac.name.map(lambda x: re.sub(r"\s+", " ", SITECODE.sub("", x)).strip(" -(),") or x)
    ct_fac = {f: [[r.name, r.city, r.zip, r.trials] for r in g.itertuples()] for f, g in fac.groupby("county_fips")}
    # ---- county metrics ----
    ...
    for f in cm.index:
        ...
        counties[f] = {"n": c.county_name, "s": str(c.state_fips).zfill(2), "p": int(c.pop55), "t": int(c.trials_in_county), "f": len(ct_fac.get(f, [])),
                       "tr": ct_trials.get(f, []), "fac": ct_fac.get(f, []), "nb": nb60.get(f, []), "t60": pool60[f], "f60": fac60[f],
    ...
    for k, r in dm.iterrows():
        ...
        districts[k] = {"st": INV[sf], "stname": st_name[sf], "cd": r.cd.lstrip("0") or "0", "p": int(r.pop55), "nt": int(r.n_tracts),
                        ...
                        "member": m["name"] if m else "", "party": m["party"] if m else "", "hr3521": "lead" if k in lead_hr3521 else ""}
    ...
    meta = {"version": "3.0", "pull": pull_date, "trials": len(trials), "facilities": int(fac.shape[0]), "plan": "119th Congress (2021 maps)", "members_pull": "2026-09-09 (unitedstates/congress-legislators, gh-pages)",
    ...
    data = {"meta": meta, "T": T, "SP": SP, "SPF": SPF, "counties": counties, "districts": districts, "nci": ncil, "states": statelist}
    ...
    geo = {"counties": simp(gc, 0.012, lambda r: r.GEOID), "districts": simp(gd, 0.007, lambda r: r.cdid, lambda r: INV.get(r.STATEFP, ""))}
